...
if CUSTOM_NAME:
    try:
        ...
        if os.path.exists(favicon_path):
            # 如果 favicon.png ⽂件存在，就直接复制到 STATIC_DIR ⽬录
            shutil.copy(favicon_path, f"{STATIC_DIR}/favicon.png")
        if os.path.exists(splash_path):
            # 如果 splash.png ⽂件存在，就直接复制到 STATIC_DIR ⽬录
            shutil.copy(splash_path, f"{STATIC_DIR}/splash.png")
        # 如果需要⾃定义名称，可以直接从 CUSTOM_NAME 设置
        WEBUI_NAME = CUSTOM_NAME if CUSTOM_NAME else "智能助⼿"
    except Exception as e:
        log.exception(e)
        pass
